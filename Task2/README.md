## Day 2 : Basic Shell and Computer Network

### Diagram jaringan komputer dengan 4 device dengan kondisi :

- IP Class C : 192.168.4.xxx
- CIDR Block : 192.168.4.0/24

### perbedaan shell dan bash

- Shell biasanya bentuknya seperti dollar saja, sedangkan bash itu ada username dan servernya
- Shell itu bentuk basic nya, sedangkan Bash ada fitur tambahannya

### Command Linux

#### sudo apt update = supaya dalam kondisi update

#### mkdir = untuk membuat folder

#### ls = untuk melihat list

#### ls -la = untuk melihat list lebih detail

#### touch = untuk membuat file

#### cd (nama folder) = masuk ke direktori

#### cd .. = mundur ke direktori belakang

#### cp = untuk copy

#### mv = untuk memindahkan file

#### echo = pantulan text

#### echo > = pantulan text masuk ke file (replace)

#### echo >> = pantulan text masuk ke file (add)

#### cat = untuk melihat isi file

#### find = menunjukan semua isi direktori

#### find -type t = menunjukan isi direktori hanya file

#### find -type d = menunjukan isi direktori hanya direktori

#### find -type f -name (nama file) = menunjukkan isi direktori seusai nama spesisfik

#### grep (kata) (nama file) = untuk mencari kata di dalam file

#### nano = masuk teks editor dalam file

#### grep -r (kata) = mencari kata dari semua file yang ada di direktori sekarang

#### chmod = untuk mengubah premision

#### chown = menukar kepemilikan

#### history = menampilkan semua commad yang pernah di input

#### sudo su = mengubah user menjadi root
